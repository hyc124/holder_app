  ...
  ; (function ($w) {
    ...
    jm.prototype = {
      ...
      click_handle: function (e) {
        ...
        if (event.srcElement.className == 'jsmind-editor') {
          //点击input 状态下不选中
          return
        }
        var element = e.target || event.srcElement
        ...
        if (isexpander) {
          ...
          (jQuery(e.target).hasClass('timeInput') || jQuery(e.target).parents('.timeInput').length > 0) &&
          tsakAuths
        ) {
          //点击时间
          isMousedown = true
          ...
          jQuery('.okrnode')
            .empty()
            .attr('nodeid', '')
            .attr('nodeid', nodeid) //缓存节点数据
          // this.invoke_event_handle(jm.event_type.select, { evt: text, data: [e], node: nodeid });
        } else if (
          (jQuery(e.target).hasClass('show_progress') || jQuery(e.target).parents('.show_progress').length > 0) &&
          tsakAuths
        ) {
          //点击进度
          ...
          jQuery('.okrnode')
            ...
            .attr('nodeid', '')
            .attr('nodeid', nodeid) //缓存节点数据
        } else if (jQuery(e.target).hasClass('confidence') && tsakAuths) {
          //信心指数
          isMousedown = true
          ...
          jQuery(e.target)
            ...
            .find('.eidt_Kr_Process')
            .show()
          jQuery('.okrnode')
            .empty()
            .attr('nodeid', '')
            .attr('nodeid', nodeid) //缓存节点数据
        } else if (jQuery(e.target).hasClass('edit_portrait') && tsakAuths) {
          //常用联系人
          isMousedown = true
          jQuery('.eidt_Kr_Process').hide()
          // e.stopPropagation()
      ...
      dblclick_handle: function (e) {
        ...
        if (
          e.srcElement.localName == 'jmexpander' ||
          isStaticmind ||
          isBreaking ||
          affiliation ||
          progress ||
      ...
      insert_node_parent: function (node_before, nodeid, topic, data) {
        //添加父节点
        if (this.get_editable()) {
          ...
          if (!!node) {
            ...
            this.layout.layout()
            this.view.show(false)
            //自动选中为编辑状态 //this.select_node(nodeid); //this.begin_edit(nodeid);
          }
        } else {
          logger.error('fail, this mind map is not editable')
          return null
      ...
      move_node: function (nodeid, beforeid, parentid, direction, beforeIndex, beforePid) {
        //拖动
        if (this.get_editable()) {
          var fromParentId = this.get_node(nodeid).parent.id
          this.invoke_event_handle(jm.event_type.edit, {
            evt: 'move_node',
            data: [nodeid, beforeid, parentid, direction, fromParentId, beforePid, beforeIndex],
            node: nodeid,
          })
          return
      ...
      select_node: function (node) {
        ...
        this.invoke_event_handle(jm.event_type.select, {
          ...
          data: [],
          node: node.id,
          tagName: this.mind.selected._data.view.element,
        })
      },
        ...
        while (i--) {
          ...
          if (node.parent.isroot) {
            ...
            layout_data.offset_x =
              (this.opts.hspace + 30) * layout_data.direction +
    ...
    jm.graph_svg.prototype = {
      ...
      draw_line: function (pout, pin, offset, node) {
        ...
        if (status == '4' || status == '5' || status == '3' || taskType == '3') {
          //派发及KR
    ...
    jm.view_provider.prototype = {
      init: function () {
        ...
        jm.util.dom.add_event(this.e_editor, 'keyup', function (e) {
          var evt = e || event
      ...
      create_node_element: function (node, parent_node) {
        ...
        if (node.isroot) {
          ...
          if (node.data.status == 4 || node.data.status == 5 || node.data.status == 3) {
            //派发及已完成
            d.className += ' accomplish'
          }
          if (node.data.taskStatus == 2) {
            //完成
            d.className += ' complete'
          }
        } else {
          var d_e = $c('jmexpander')
          if (node.data.taskType == '2' || node.data.taskType == '3') {
            //okr
          ...
          if (node.data != undefined && node.data != null && node.data != '') {
            if (node.data.status == 5 || node.data.taskStatus == 2) {
              //完成
              d.className += ' accomplish complete'
            } else if (node.data.status == 4 || node.data.status == 3) {
              //派发
              d.className += ' accomplish'
            }
          }
      ...
      confidenceadd_task: function (node, className) {
        ...
        if (node.isroot) {
          ...
          if (node.parent.data.taskType == 2 && !node.parent.isroot) {
            //第二层且父级为O的状态下可以添加父级KR
            isAddKR = 'style="display:none"'
          }
        }
        // 添加上下左右新增任务
        if (node.isroot) {
          //o
          if (node.data.taskType == 2) {
            //第一层为O的状态下不可以新增父级 共享点击下第一层也不增加父级
      ...
      edit_node_end: function () {
        if (this.editing_node != null) {
          ...
          jQuery('.okrnode')
            ...
            .attr('nodeid', '')
            .attr('nodeid', node.id) //缓存节点数据
          this.jm.invoke_event_handle(jm.event_type.select, {
            evt: 'edit_text',
            data: [],
            node: node.data.nodeId,
            type: node.data.typeId,
      ...
      _show: function () {
        ...
        console.log('_show:', this.size.w, this.size.h)
        this.show_nodes()
        this.show_lines()
        //this.layout.cache_valid = true;
        this.jm.invoke_event_handle(jm.event_type.resize, { data: [] })
        //渲染脑图记录上一次放大缩小
        const scalenum = jQuery('.scale-zoom i')
          .text()
          .split('%')[0]
      ...
      show_nodes: function () {
        ...
        for (var nodeid in nodes) {
          ...
          for (var i = 0; i < node.children.length; i++) {
            if (
              node.children[i].data.status == 4 ||
              node.children[i].data.status == 3 ||
              node.children[i].data.status == 5
            ) {
              _status = 4 //派发
              break
            }
          ...
          } else if (node.data.hasChild > 0 && node.children.length == 0) {
            //判断是否有子任务 (加减号)
            expander_text = '+'
            p_expander = this.layout.get_expander_point(node)
            expander.style.left = _offset.x + p_expander.x + 3 + 'px'
    ...
    jm.shortcut_provider.prototype = {
      ...
      handle_delnode: function (_jm, e) {
        ...
        if (!!selected_node && !selected_node.isroot) {
          if (isremove) {
            this.jm.invoke_event_handle(jm.event_type.edit, {
              evt: 'remove_node',
              data: [selected_node.id],
              node: selected_node.data.nodeId,
              type: selected_node.data.typeId,
