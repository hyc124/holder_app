...
; (function ($w) {
  ...
  jcanvas.text_ellipsis = function (ctx, text, x, y, w, h) {
    var center_y = y + h / 2
    const texts = jcanvas.fittingString(ctx, text, w)
    ctx.textAlign = 'left'
    ctx.textBaseline = 'middle'
    ctx.fillText(texts, x, center_y, w)
  }
  ...
  jsMind.screenshot.prototype = {
    ...
    _draw_node: function (node) {
      ...
      var rb = { x: view_data.abs_x, y: view_data.abs_y, w: view_data.width + 1, h: view_data.height + 1 }
      var tb = {
        x: rb.x + padding_left,
        y: rb.y + padding_top,
