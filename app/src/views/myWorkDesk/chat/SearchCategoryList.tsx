...
import { queryAddressList } from '@/src/views/chatwin/getData/getData'
...
let count = 0
const SearchCategoryList: React.FC<ParamProps> = ({ keywords, refcallback }) => {
  ...
  //加载更多群组数据与已有群组数据 去除重复数据
  const repeatedlyData = (groups: any) => {
    const nowRoomData = roomListState.groupData.map((item: any) => {
      return item.id
    })
    const noRepeat = groups.filter(($item: any) => !nowRoomData.includes($item.id))
    return noRepeat
  }
...
export default memo(SearchCategoryList)
