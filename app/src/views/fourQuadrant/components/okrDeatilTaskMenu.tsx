...
//取消O选择任务归属
let tasktypes = 0
const OkrDeatilTaskMenu = (taskList: any) => {
  ...
  const [replayVisble, setReplayVisble] = useState({
    nowVisble: false, //控制复盘弹窗
    deleteVisable: false, //控制二次确认删除弹窗
    code: '',
    getData: {},
    isAll: 0,
  })
  ...
  useEffect(() => {
    if (taskList) {
      ...
      contentData.forEach((item: CodeItem) => {
        newTableData.push({
          code: item.code,
          status: item.status,
          name: item.name,
  ...
  const taskItemOpth = (listData: any, data: any, callBtn: any) => {
    ...
    listData.map((item: CodeItem, index: number) => {
      if (item.name === 'EXECUTE' && item.status != 0) {
        ...
        // } else if (item.code == 12 || item.code == 13) {
        //   sendHtml.push(item)
        // } else if (item.name === 'SHARE_PERSON' || item.name === 'SHARE_GROUP') {
        //   shareHtml.push(item)
      } else if (item.name === 'ICON') {
    ...
    switch (Number(paramObj.code)) {
      case 1:
        return (
          ...
          >
            <div className="myMenuItem" style={{ paddingLeft: '19px' }}>
      ...
      case 3:
        ...
        return (
          ...
          >
            <div className="myMenuItem">
              <span style={{ marginLeft: '43px' }}>{$intl.get('Report')}</span>
            </div>
          </Menu.Item>
        )
      case 5:
          ...
          <Menu.Item
            ...
            onClick={() => {
              setAssignParam({
                ...
                ascriptionId: gettaskData.ascriptionId,
                fromType: 'mind-map',
                callback: (taskIds: any) => {
                  callBtn(paramObj.code, '', taskIds)
                },
              })
      ...
      case 100: //详情
        // 草稿任务展示详情按钮
        return getData.type == 1 && getData.status == 1 ? (
          <Menu.Item
            key={index}
            className="opt_icon_div "
            disabled={paramObj.isUse ? false : true}
      ...
      case 113:
        return (
          ...
          >
            <div className="myMenuItem">
              <span style={{ marginLeft: '27px' }}>{$intl.get('movePlan')}</span>
            </div>
          </Menu.Item>
        )
...
export default OkrDeatilTaskMenu
