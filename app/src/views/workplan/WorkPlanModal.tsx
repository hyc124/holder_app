...
import { moveToDirSave, delFolder, delPlan, shareToRoomSave } from './WorkPlanOpt'
...
export const WorkPlanModal = (props: any) => {
  ...
  const geteItem: any = $store.getState().planModalObj.sourceItem || {}
  return (
    <section>
      ...
      <DelPlanModal
        ...
        refreshData={planContextObj.refreshData}
      />
    ...
    return (
      <div className={'cellCont flex between'}>
        ...
        <div className="rowRight">
          <span className={`now_pos_msg ${folderPos.id == item.id ? '' : 'forcedHide'}`}>当前所在位置</span>
        </div>
      </div>
    )
  }
...
export const MovePlanModal = (props: { props: any }) => {
  ...
  return (
    ...
    >
      <MoveToDirlist
        visible={visible}
        planType={planContextObj.planType}
      ...
      >
        ...
        <div className="rowRight">
          <span className={`now_pos_msg ${folderPos.id == '' ? '' : 'forcedHide'}`}>当前所在位置</span>
        </div>
      </div>
    </Modal>
  )
 ...
 * 删除规划或文件夹二次弹框提醒
 */
export const DelPlanModal = (props: any) => {
  const getProps = props.props
  const param = getProps.param
  const action = getProps.action
  const sourceItem = props.dataItem
  const [visible, setVisible] = useState(false)
  const handleOk = () => {
    action.setDelPlanModalShow(false)
    if (sourceItem.viewType == 1) {
      delFolder(
        {
          id: sourceItem.id, //目标文件夹id
        },
        {
          planContextObj: { refreshData: props.refreshData },
        }
      )
    } else {
      delPlan(
        {
          ...
          isDel: sourceItem.isDel, //是否删除O下面及所有任务
          firstId: sourceItem.parentId, //第一个根节点ID
          mainTypeId: sourceItem.mainTypeId, //根节点typeid
          isAll: sourceItem.isAll == 0 ? 0 : 1,
        },
        {
          planContextObj: { refreshData: props.refreshData },
        }
  ...
  useEffect(() => {
    setVisible(param.delPlanModalShow)
  }, [param.delPlanModalShow])
  return (
    <Modal
    ...
    >
      {sourceItem.type == 1 && sourceItem.hasChild > 0 && (
        <p className="contTxt">
          删除该任务及其包含的子任务？
          <br />
          <br />
          (当前任务包含<span>{sourceItem.hasChild}</span>个子任务，请谨慎操作。)
        </p>
      )}
      {sourceItem.type == 1 && !sourceItem.hasChild && (
        <p className="contTxt">
          点击后系统将永久删除【{sourceItem ? sourceItem.name : ''}】！
          <br />
          <br />
          如果该任务下有其他参与者或子任务
          <br />
      ...
      {(sourceItem.type == 2 || sourceItem.type == 3) && (
        <p className="contTxt">
          删除{sourceItem.type == 2 ? '目标' : 'KR'}后无法找回,【{sourceItem ? sourceItem.name : ''}】?
        </p>
      )}
      {(sourceItem.type == 0 || sourceItem.type == undefined) && (
        <p className="contTxt">删除计划会连同任务一起删除，且无法找回，确定删除该计划？</p>
      )}
    </Modal>
  )
}
...
export const ShareToRoomModal = (props: {
  ...
  // 标题
  const titName = param.titName ? param.titName : '选择群'
  return (
    <Modal
...
export const ShareRoomList = (props: any) => {
  ...
  return (
    <ul className="shareRoomList flex wrap">
      {!roomList || (roomList && roomList.length == 0 && <NoneData />)}
      {roomList &&
        roomList.length > 0 &&
