  ...
  ; (function ($w) {
    ...
    jm.prototype = {
      ...
      mousedown_handle: function (e) {
        ...
        if (!!nodeid) {
          if (element.tagName.toLowerCase() == 'jmnode' || $(element).parents('jmnode')) {
      ...
      add_node: function (parent_node, nodeid, topic, data, idx, direction) {
        if (this.get_editable()) {
          var node = this.mind.add_node(parent_node, nodeid, topic, data, idx, direction)
          if (!!node) {
      ...
      update_node: function (nodeid, topic, data) {
        if (this.get_editable()) {
    ...
    jm.graph_svg.prototype = {
      ...
      draw_line: function (pout, pin, offset, node) {
        ...
        // this._bezier_to_new(line, pin.x + offset.x, pin.y + offset.y, pout.x + offset.x, pout.y + offset.y, node)
      },
      ...
      _bezier_to: function (path, x1, y1, x2, y2, node) {
        if (this.opts.lineStyle == 'polyline') {
          //折线
    ...
    jm.view_provider.prototype = {
      ...
      create_node_element: function (node, parent_node) {
        ...
        //计算一行字符串的像素长度
        const getTextWidth = function (str, fontSize) {
          let result = 0
        ...
        new Promise(resolve => {
          ...
          const textW = getTextWidth(node.data.names, '12px')
          if (textW >= 220) {
            d.style.height = '96px'
          } else {
            d.style.height = '90px'
      ...
      setZoom: function (zoom, obj, type) {
        ...
        this.jm.options.scale_view(zoom)
        if (obj != '') {
      ...
      show_nodes: function () {
        ...
        for (var nodeid in nodes) {
          ...
          if ((rootCheck && node.children.length > 0) || (node.data.hasChild > 0 && node.children.length == 0)) {
            // 修改源码标记处:
          ...
          if (rootCheck && node.children.length == 0 && node.data.hasChild == 0) {
            expander.style.display = 'none'
